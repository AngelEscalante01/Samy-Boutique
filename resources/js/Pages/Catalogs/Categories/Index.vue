...
function submit() {
  form
    ...
    .post(route('catalogs.categories.store'), {
      preserveScroll: true,
...
<template>
  ...
  <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
      ...
      <span class="font-medium text-gray-700">Categorías</span>
    </nav>
    ...
    <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
        ...
        <form @submit.prevent="submit" class="flex flex-wrap items-start gap-3">
          ...
          <div class="flex flex-col gap-1 min-w-[200px] flex-1">
            <input
              v-model="form.name"
              type="text"
              placeholder="Nombre del categoría"
      ...
      <CatalogTable
        :items="categories"
        update-route="catalogs.categories.update"
        toggle-route="catalogs.categories.toggle"
        delete-route="catalogs.categories.destroy"
        :show-hex="false"
      />
      ...
      >
        <p class="text-xs text-gray-500">
          ...
          &middot; {{ categories.total }} registros
        </p>
