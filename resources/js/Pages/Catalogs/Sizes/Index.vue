...
function submit() {
  form
    ...
    .post(route('catalogs.sizes.store'), {
      preserveScroll: true,
...
<template>
  ...
  <div class="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8 space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
      ...
      <span class="font-medium text-gray-700">Tallas</span>
    </nav>

    <!-- ── Encabezado ─────────────────────────────────────────────────────── -->
    <div>
      <h1 class="text-2xl font-bold text-gray-900">Tallas</h1>
    ...
    <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-200">
      ...
      <CatalogTable
        :items="sizes"
        update-route="catalogs.sizes.update"
        toggle-route="catalogs.sizes.toggle"
        delete-route="catalogs.sizes.destroy"
        :show-hex="false"
      />

      <!-- Paginación -->
      <div
        v-if="sizes.last_page > 1"
        class="flex items-center justify-between border-t border-gray-100 px-5 py-3 bg-gray-50/40"
      >
        <p class="text-xs text-gray-500">
          Página {{ sizes.current_page }} de {{ sizes.last_page }}
          &middot; {{ sizes.total }} registros
        </p>
        <div class="flex gap-1.5">
          <Link
            v-if="sizes.prev_page_url"
